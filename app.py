...
# --- IDs FIJOS ---
# Estos IDs son fijos para Vehículos y Configuración. El Roster se busca dinámicamente.
...
# --- FUNCIÓN CLAVE: CARGA TRABAJADORES FILTRANDO YA FICHADOS ---
def cargar_trabajadores(id_roster, dia_mes):
    ...
    try:
        ...
        # Buscar columna del día en cabeceras
        for r in range(3, 9):
            if r < len(datos):
                if str_dia in datos[r]: 
        ...
        for fila in datos[8:]:
            ...
            # SI YA TIENE DATO EN ESE DÍA, LO SALTAMOS
            if len(fila) > col_dia:
                val_dia = str(fila[col_dia]).strip()
                if val_dia and val_dia not in ["", "None"]:
                    continue 
...
def buscar_columna_dia_idx(ws, dia_num):
    # Devuelve índice 1-based para gspread
...
with st.sidebar:
    ...
    if conf_prod:
        ...
        if tramo_sel:
            ...
            st.session_state.ARCH_PROD = conf_prod.get(tramo_sel)
...
with t1:
    if st.session_state.ID_ROSTER:
        ...
        # Cargamos trabajadores pasando el día para filtrar los ya fichados
        with st.spinner("Buscando personal disponible..."):
            trabs = cargar_trabajadores(st.session_state.ID_ROSTER, d)
        ...
        if st.button("➕ AÑADIR", type="secondary", use_container_width=True):
            if trab_sel and trab_sel != "" and trab_sel != "Sin personal":
                    ...
                    if it:
                        ...
                        if not typs: c2.write("-")
                            ...
                            for c_idx in cols_escritura:
                                guardar_prod_con_nota(nom, hj, fr, c_idx, hoy, st.session_state.veh_glob)
                            if it not in st.session_state.prod_dia: st.session_state.prod_dia[it]=[]
